...
create_vignette_images <- function(df, output_dir = "vignettes_images") {
  ...
  for (i in seq_len(nrow(df))) {
    ...
    text_locations <- list(
      park = "+475+320",       # Position for park image
      shopping = "+475+190",   # Position for shopping image
    ...
    vignette_img <- base_img %>%
      image_annotate(vignette_text, size = 30, color = "black", location = text_location, font = "Arial")
