...
#Model 5: Scale + acceptance
model_5 <- lmer(FRT_acceptance_composite ~ scale + (1 | prolific_id), data = data_excl_both_mani_failures)
summary(model_5)

#Model 6: Including all randomised variables
#TRY AGAIN - ADD ALL INTERACTIONS
model_6 <- lmer(FRT_acceptance_composite ~ scenario + actor + type + scale (1 | prolific_id), data = data_excl_both_mani_failures)
summary(model_6)

#ACME:
#FINISH ADJUSTING THIS
avg_comparisons(
  model_6,                         
  variables = "actor",
  vcov = ~prolific_id,
